...
describe('CategorySequelizeRepository Integration specs', () => {
  ...
  it('should find a category by id', async () => {
    ...
    let entityFound = await repository.findById(category.categoryId);
  ...
  it('should find all categories', async () => {
    const firstCategory = Category.fake().aCategory().build();
    const secondCategory = Category.fake().aCategory().build();

    let categories = await repository.findAll();
    expect(categories).toHaveLength(0);

    await CategoryModel.create({
      category_id: firstCategory.categoryId.id,
      name: firstCategory.name,
      description: firstCategory.description,
      is_active: firstCategory.isActive,
      created_at: firstCategory.createdAt,
    });
    ...
    await CategoryModel.create({
      category_id: secondCategory.categoryId.id,
      name: secondCategory.name,
      description: secondCategory.description,
      is_active: secondCategory.isActive,
      created_at: secondCategory.createdAt,
    });
